...
export class TaxService {
    private static cache: Record<string, TaxConfig> = {};

    static async getTaxConfig(countryCode: string = 'US'): Promise<TaxConfig | null> {
        // Serve from in-memory cache (refreshed every 10 minutes)
        if (this.cache[countryCode]) return this.cache[countryCode];

        try {
            const repo = AppDataSource.getRepository(TaxConfig);
            const config = await repo.findOne({ where: { countryCode, isActive: true } });
            if (config) this.cache[countryCode] = config;
            return config;
        } catch {
            return null;
        }
    ...
    static async calculateOrderTotal(
        ...
        serviceClass: string = 'Economy',
        distanceInKm?: number,
        durationInMinutes?: number,
        surgeMultiplier: number = 1.0
    ): Promise<{ price: number; tax: number; total: number; currency: string; currencySymbol: string; taxName: string }> {
    ...
    static clearCache(countryCode?: string) {
