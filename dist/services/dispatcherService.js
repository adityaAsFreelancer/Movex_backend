...
class DispatcherService {
     ...
     */
    static async findBestDriver(order, io) {
        ...
        const config = configs[order.serviceClass] || configs['Economy'];
        const logs = order.dispatchLogs || [];
        const log = (msg) => {
            const entry = { timestamp: new Date(), message: msg };
            logs.push(entry);
            if (io)
                io.emit('dispatch_log', { orderId: order._id, ...entry });
        };
        log(`AI Dispatcher [v2.5]: Protocol ${order.serviceClass} sweep initiated...`);
        const pLat = order.pickupCoords?.lat;
        const pLng = order.pickupCoords?.lng;
        if (!pLat || !pLng) {
        ...
        if (retryCount >= 5) {
            ...
            (0, notificationService_1.sendNotification)(order.customerId?._id, 'Mission Aborted', 'No pilots available in sector. Refund processed.', { orderId: order._id });
            return null;
        }
        ...
        const scored = (await Promise.all(candidates.map(async (row) => {
            const driver = userRepository.create(row);
            let score = 100;
            // Distance Penalty
        ...
        (0, notificationService_1.sendNotification)(best._id, 'Mission Request', `New ${order.serviceClass} mission available. Accept now!`, { orderId: order._id, type: 'MISSION_ASSIGNED' });
        if (io)
     ...
     */
    static async processQueuedMissions(io) {
        const orderRepository = data_source_1.AppDataSource.getRepository(Order_1.Order);
